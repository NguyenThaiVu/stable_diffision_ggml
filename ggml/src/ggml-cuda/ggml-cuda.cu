...
static void ggml_cuda_op_mul_mat_cublas(
    ...
    // DEBUG print
    // printf("[DEBUG] Entering ggml_cuda_op_mul_mat_cublas\n");
    ...
    if (supports_bf16 && src0->type == GGML_TYPE_BF16 && ggml_is_contiguous(src0) && row_diff == src0->ne[1]) {
        // printf("[DEBUG]: Using BF16 cuBLAS path\n");
        ...
        // printf("[DEBUG]: Using FP16 cuBLAS path\n");
...
// ================================================================================================
/*
This is my custom kernel for cuBLAS matmul
*/
static void custom_ggml_cuda_op_mul_mat_cublas(
    ggml_backend_cuda_context & ctx,
    const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst, const char * src0_dd_i, const float * src1_ddf_i,
    const char * src1_ddq_i, float * dst_dd_i, const int64_t row_low, const int64_t row_high, const int64_t src1_ncols,
    const int64_t src1_padded_row_size, cudaStream_t stream) {

    // printf("\n[DEBUG]: My Custom int8 CuBLAS Kernel\n");
    // print_ggml_tensor_info(src0, "src0");
    // print_ggml_tensor_info(src1, "src1");
    // print_ggml_tensor_info(dst, "dst");
    // printf("\n");
    ...
    // Allocate temporary buffers for src0 and src1 in fp32 format if they are not already in fp32
    ggml_cuda_pool_alloc<float> src0_ddq_as_f32(ctx.pool(id));
    ggml_cuda_pool_alloc<float> src1_ddq_as_f32(ctx.pool(id));
    ...
    // Convert src1 to int8
    ggml_cuda_pool_alloc<int8_t> src1_as_i8(ctx.pool(id));
    ggml_cuda_pool_alloc<float>  src1_col_scales(ctx.pool(id));
    src1_as_i8.alloc(ne10 * src1_ncols);
    src1_col_scales.alloc(src1_ncols);

    quantize_fp32_to_int8_col_wise_cuda(
        src1_ddf1_i,
        src1_as_i8.get(),
        src1_col_scales.get(),
        ne10,        // rows
    ...
    CUBLAS_CHECK(
        cublasGemmEx(ctx.cublas_handle(id),
            ...
            ne10,
            &alpha_i32,
            src0_as_i8.get(), CUDA_R_8I,  ne00,
            src1_as_i8.get(), CUDA_R_8I,  ne10,
            &beta_i32,
            dst_i32.get(),    CUDA_R_32I, ldc,
            CUBLAS_COMPUTE_32I,
            CUBLAS_GEMM_DEFAULT));

    // Dequantize int32 result back to fp32 with per-row and per-column scales
...
- src1 in fp32
*/
static void custom_ggml_q8_kernel(
    ggml_backend_cuda_context & ctx,
    const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst, const char * src0_dd_i, const float * src1_ddf_i,
    const char * src1_ddq_i, float * dst_dd_i, const int64_t row_low, const int64_t row_high, const int64_t src1_ncols,
    const int64_t src1_padded_row_size, cudaStream_t stream) {

    // printf("[DEBUG]: My Custom int8 MMQ + CuBLAS Kernel\n");
    // print_ggml_tensor_info(src0, "src0");
    // print_ggml_tensor_info(src1, "src1");
    // print_ggml_tensor_info(dst, "dst");
    // printf("\n");
    ...
    */
    ...
    // // ------------------------------------------------------------
    // // Step 1: convert src0 Q8_0 -> int8 + scales
    // // ------------------------------------------------------------
    ggml_cuda_pool_alloc<int8_t> src0_as_i8(ctx.pool(id));
    ggml_cuda_pool_alloc<float>  src0_scales(ctx.pool(id));
    ...
    // ------------------------------------------------------------
    // Step 2: quantize src1 fp32 -> int8 + scales
    // ------------------------------------------------------------
    ggml_cuda_pool_alloc<int8_t> src1_as_i8(ctx.pool(id));
    ...
    // The usage of row_wise function is correct.
    // Because the math is dst = src0 @ src1^T
    quantize_fp32_to_int8_row_wise_cuda(
        src1_ddf_i,
        src1_as_i8.get(),
        src1_scales.get(),
    ...
    // ------------------------------------------------------------
    // Step 3: cuBLAS int8 x int8 -> int32
    // ------------------------------------------------------------
    ggml_cuda_pool_alloc<int32_t> dst_i32(ctx.pool(id));
    dst_i32.alloc(ldc * N);
    ...
    // // Using CUTLASS for int8 matmul
    // bool matmul_success = int8_matmul_cutlass_cuda(
    //     src0_as_i8.get(),  // A: [row_diff, K], row-major
    //     src1_as_i8.get(),  // B: [N, K], row-major physical memory
    //     dst_i32.get(),     // C: [row_diff, N], column-major physical memory
    //     row_diff,          // M
    //     N,                 // N
    //     K,                 // K
    ...
    // ------------------------------------------------------------
    // Alternative Step 3 & 4: custom CUDA kernel for i8i8 -> fp32 with dequantization
    // ------------------------------------------------------------
...
static void custom_ggml_q4_kernel_BACKUP(
    ggml_backend_cuda_context & ctx,
...
static void custom_ggml_q4_kernel(
    ggml_backend_cuda_context & ctx,
    const ggml_tensor * src0,
    const ggml_tensor * src1,
    ggml_tensor * dst,
    ...
    // printf("[DEBUG]: My Custom Q4_0 -> INT8 CUTLASS Kernel\n");
    // print_ggml_tensor_info(src0, "src0");
    // print_ggml_tensor_info(src1, "src1");
    // print_ggml_tensor_info(dst,  "dst");
    // printf("\n");
...
static void ggml_cuda_mul_mat(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    if (!split && use_mul_mat_vec_f) {
        ...
        // NOTE: this path is used for src0 (Q4_0, Q8_0...) and src1 (F32)
        // We can modify this to support our custom quantization kernel.
        if (use_custom_kernel()) {
            if (src0->type == GGML_TYPE_Q8_0) {
            printf("Using custom Q8 kernel for mul_mat\n");
            ggml_cuda_op_mul_mat(ctx, src0, src1, dst, custom_ggml_q8_kernel, nullptr);
            } else if (src0->type == GGML_TYPE_Q4_0) {
                printf("Using custom Q4 kernel for mul_mat\n");
                // ggml_cuda_op_mul_mat(ctx, src0, src1, dst, custom_ggml_q4_kernel, nullptr);
                ggml_cuda_op_mul_mat(ctx, src0, src1, dst, custom_ggml_q4_kernel_spin, nullptr);
            } else {
                ggml_cuda_mul_mat_q(ctx, src0, src1, nullptr, dst);  // Fallback to default path if no custom kernel is available for the type.
            }
        } else {
            ggml_cuda_mul_mat_q(ctx, src0, src1, nullptr, dst);  // Default path of GGML.
        }
        ...
        // ggml_cuda_op_mul_mat(ctx, src0, src1, dst, custom_ggml_cuda_op_mul_mat_cublas, nullptr);
    }
}
